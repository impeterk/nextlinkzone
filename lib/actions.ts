...
export async function createNewPage(formData: FormData) {
  ...
  try {
    ...
    pagename = z
      .object({
        pagename: z
          .string()
          .min(5, 'please provide at least 5 charactes')
          .max(20, 'please provide maximumm 20 characters'),
      })
      .parse({
...
export async function createNewLink(details: {
  name: string;
  href: string;
  pageId: string;
  icon?: string;
...
export async function setNewPageImage(pageId: string, img: string | null) {
  const session = await auth();
  ...
  try {
    ...
    if (page.image === img) return false;
    await db
      .update(pages)
      .set({ image: img })
      .where(eq(pages.id, pageId))
